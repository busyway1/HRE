...
Private Sub Select_Cmd_Click()
    ...
    If Not foundRow Is Nothing Then
        If Me.File_Option = True Then
            If foundRow.Cells(1, tbl.ListColumns("구분").Index - 1).Value = "파일" Then
                ' 파일 확장자 검사
                ...
                End If
                foundRow.Cells(1, tbl.ListColumns("경로").Index - 1).Value = Me.Path_Text.Value
            Else
                Msg "선택한 항목이 '파일'이 아닙니다.", vbExclamation
                Exit Sub
            End If
        ElseIf Me.Folder_Option = True Then
            If foundRow.Cells(1, tbl.ListColumns("구분").Index - 1).Value = "폴더" Then
                ' 폴더 경로 검사
...
Private Sub Name_Combo_Change()
    ...
    If Not foundCell Is Nothing Then
        ...
        presentPath = tbl.ListColumns("경로").DataBodyRange.Cells(foundCell.row - dataRange.row + 1, 1).Value
