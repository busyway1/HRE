...
Sub RefreshPivotVerify()
    ...
    Call SpeedUp
    Call OpenProgress("ǹ ΰħ ...")
       
    For Each pt In Verify.PivotTables
        pt.RefreshTable
    Next pt

    Call CalculateProgress(1, "ΰħ Ϸ")
    Call SpeedDown
    Set pt = Nothing
End Sub
Sub VerifyBS()
    ...
    Set pvt = Verify.PivotTables("ջ(BS)")
    ...
    For i = dataRange.row + 2 To dataRange.row + dataRange.Rows.count - 1
        ...
        Set foundCell = linkTable.ListColumns("ڵ").DataBodyRange.Cells.Find(What:=corpCode, LookAt:=xlWhole, MatchCase:=False)
...
Sub VerifyIS()
    ...
    Set pvt = Verify.PivotTables("ջ(IS)")
    Set dataRange = pvt.TableRange1
    Set linkTable = HideSheet.ListObjects("Link")
    Set corpTable = CorpMaster.ListObjects("Corp")
    Set dateTable = HideSheet.ListObjects("꿬")
    ...
    For i = dataRange.row + 2 To dataRange.row + dataRange.Rows.count - 1
        ...
        Dim cell As Range
        For Each cell In corpTable.ListColumns("ڵ").DataBodyRange
        ...
        '  
        formula = "=IF((J" & i & "-K" & i & "-XLOOKUP(I" & i & ",[ڵ],[ݾ],,0)) = 0, ""TRUE"", J" & i & "-K" & i & "-XLOOKUP(I" & i & ",[ڵ],[ݾ],,0))"
        With Verify.Cells(i, verifyCol)
            .formula = formula
            .Font.name = "  Semilight"
            .Font.Size = 11
...
Sub ValidateCorpCodes()
    ...
    Set corpTable = CorpMaster.ListObjects("Corp")
    Set linkTable = HideSheet.ListObjects("Link")
    Set corpCodeColumn = corpTable.ListColumns("ڵ").DataBodyRange
    ...
    With Verify.Cells(6, 6)
        ...
        bsRange = Verify.PivotTables("ջ(BS)").RowRange.Address
        .formula = "=COUNTA(" & bsRange & ")-1"
    End With
    
    With Verify.Cells(7, 6)
        Dim isRange As String
        isRange = Verify.PivotTables("ջ(IS)").RowRange.Address
    ...
    If isVerified Then
        MsgBox "ܼ ڷᴩ  ϷǾϴ." & vbNewLine & "κ  Ȯϼ.", vbInformation, AppName & " " & AppType
    Else
        MsgBox "ܼ ڷᴩ  Ͽϴ." & vbNewLine & "  ڼ Ȯϼ." & vbNewLine & _
                " κ  Ȯϼ.", vbExclamation, AppName & " " & AppType
    End If
...
Sub ValidateSheetColors()
    ...
    If invalidColorFound Then
        MsgBox " Ͽϴ.  Ȯּ.", vbExclamation, AppName & " " & AppType
    Else
        With Check.Cells(20, 4)
            .Value = "Complete"
            .Interior.Color = RGB(198, 239, 206)
            ...
            .Offset(0, 2).Value = GetUserInfo()
        End With
        MsgBox " ϷǾϴ.", vbInformation, AppName & " " & AppType
    End If
