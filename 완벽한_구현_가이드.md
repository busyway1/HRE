...
#### 단계 2: 헤더 행 (B4:J4) - **HRE v1.00 업데이트: 9컬럼**

> **BEP 대비 변경사항**: 법인명 컬럼 추가 (C4)
...
| 컬럼 | 인덱스 | 형식 | 소스 |
|------|--------|------|------|
| 법인코드 (B) | 1 | 텍스트 | Power Query (메타데이터 점 이전) |
| **법인명 (C)** | **2** | 텍스트 | **Power Query (메타데이터 점 이후 또는 폴더명)** |
| 법인별CoA (D) | 3 | 텍스트 | Power Query |
| 법인별계정과목명 (E) | 4 | 텍스트 | Power Query |
| PwC_CoA (F) | 5 | 텍스트 | VBA 자동 매핑 |
| PwC_계정명 (G) | 6 | 텍스트 | VBA 자동 매핑 |
| 대분류 (H) | 7 | 텍스트 | VBA 자동 매핑 |
| 중분류 (I) | 8 | 텍스트 | VBA 자동 매핑 |
| 금액 (J) | 9 | 통화 | Power Query (당기 = 차변 - 대변) |

**중요**: 이 테이블은 **Power Query로 자동 채워집니다** (빈 상태로 두세요).

**✅ 성공 확인**:
- 테이블 이름 "PTB" 확인
- **9개 컬럼** 헤더 정확히 일치 (법인명 컬럼 포함)
- B5 셀 빈 상태

### 4.6 HideSheet 시트 - 설정 테이블 (6개)

HideSheet는 **6개의 별도 테이블**을 포함합니다:

> **HRE v1.00 업데이트**: Path, TempPath 테이블 추가 (Power Query 참조용)
...
#### 테이블 3: Path (E1:E2) - **Power Query 메인주소 참조용** ⭐ 신규
...
2. **Ctrl + T**
3. 테이블 이름: `Path`
4. **용도**: Power Query `메인주소` 쿼리에서 참조

> **참고**: Link 테이블의 E열과 동일 위치에 있으며, Power Query 전용으로 사용

#### 테이블 4: TempPath (H1:H2) - **Power Query 회사결산 경로** ⭐ 신규

| H1 |
|----|
| 경로 |
| /Shared Documents/★시연용폴더★/2512 |

**테이블 생성**:
1. H1:H2 범위 선택
2. **Ctrl + T**
3. 테이블 이름: `TempPath`
4. **용도**: Power Query `회사결산_주소` 쿼리에서 참조

**경로 설정 방법**:
- **frmDirectory 폼**: 사용자가 SPO 폴더를 탐색하여 선택 시 자동 업데이트
- **수동 입력**: H2 셀에 직접 경로 입력 (슬래시 `/` 사용)

**경로 형식 예시**:
```
/Shared Documents/★시연용폴더★/2512       ← 시연용
/Shared Documents/Financial Data/2512    ← 실제 운영용
```
...
**추가 범위 설정**:
- **S2 셀**: 버전 정보 표시 위치 (VBA가 자동으로 버전 번호 입력)

**✅ 성공 확인**:
- HideSheet에 **6개 테이블** 존재
- 테이블 이름: 결산연월, Link, **Path**, **TempPath**, 비경상적, 환율마스터
- 결산연월에 2026, 1 데이터 입력됨
- Path에 SharePoint URL 입력됨
- TempPath에 결산 폴더 경로 입력됨
...
#### 단계 2: 헤더 행 (B4:I4) - **HRE v1.00 업데이트: 8컬럼**

> **BEP 대비 변경사항**: 법인명 컬럼 추가 (C4)

| B4 | C4 | D4 | E4 | F4 | G4 | H4 | I4 |
|----|----|----|----|----|----|----|----|
| 법인코드 | **법인명** | 법인별CoA | 법인별계정과목명 | PwC_CoA | PwC_계정과목명 | 적요 | 비고 |
...
- PTB 테이블에서 **6컬럼** (법인코드, 법인명, 법인별CoA, 법인별계정과목명, PwC_CoA, PwC_계정명) 복사

**✅ 성공 확인**:
- 테이블 이름 "CoA_Input" 확인
- **8개 컬럼** 헤더 정확히 일치 (법인명 컬럼 포함)
...
> **HRE v1.00 전면 재작성**: 상세 내용은 `PowerQuery/HRE_PowerQuery_전체.md` 참조

### 10.1 Power Query란?

Excel의 **데이터 연결 및 변환 도구**로, SharePoint의 다중 법인 폴더에서 시산표 파일을 자동으로 수집하고 PTB 테이블로 병합합니다.

**HRE 워크플로**:
```
HideSheet 설정 테이블 (Path, TempPath)
          ↓
SharePoint 다중 법인 폴더 순회 (Raw_Data)
          ↓
시산표 파일 내 메타데이터에서 법인코드/법인명 추출 (TB)
          ↓
PTB 테이블 생성 (9컬럼) → BSPL 시트 로드
          ↓
VBA 자동 CoA 매핑
```

### 10.2 쿼리 아키텍처 (HRE v1.00)
...
[Phase 2: 데이터 수집 - 연결 전용]
  └── Raw_Data
        ├── 의존: 메인주소, 회사결산_주소
        └── 출력: 법인명_폴더, Name, Content

[Phase 3: 데이터 변환 - 연결 전용]
  ├── fnProcessSingleFile (함수)
  └── TB
...
| 항목 | BEP | HRE v1.00 |
|------|-----|-----------|
| **법인코드 추출** | 파일명 패턴 `시산표_HRE-001_...` | 메타데이터 점 이전: `1000.xxx` → `1000` |
| **법인명** | 없음 | 메타데이터 점 이후 또는 폴더명 fallback |
| **PTB 컬럼** | 8개 | **9개** (법인명 컬럼 추가) |
| **경로 설정** | 하드코딩 | HideSheet.TempPath 테이블 참조 |
| **쿼리 개수** | 2개 | 6개 (모듈화) |

### 10.4 쿼리 생성 순서 (의존성 기반)

⚠️ **반드시 아래 순서대로 생성하세요!**

#### 순서 1: fnProcessSingleFile (함수)

1. **데이터** → **데이터 가져오기** → **기타 원본에서** → **빈 쿼리**
2. **고급 편집기**에 아래 코드 붙여넣기:
...
> **에러 방지**: `Record.FieldValues` 사용으로 컬럼명이 "Path"가 아니어도 동작합니다.
...
### 10.5 SharePoint 인증 설정

#### 첫 연결 시 인증:

1. Power Query가 SharePoint 연결 시도
2. **인증 필요** 대화상자 표시
3. **조직 계정** 선택 → **로그인**
...
**✅ 성공 확인**:
- PTB 테이블에 **9컬럼** 데이터 채워짐
- 법인코드, **법인명**, 법인별CoA, 법인별계정과목명 데이터 표시
- PwC_CoA ~ 금액은 null 상태 (VBA 매핑 후 채워짐)
