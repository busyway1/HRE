...
| 순서 | 시트 이름 | 초기 상태 | 표시 여부 | 용도 |
|------|-----------|-----------|-----------|------|
| 1 | Guide | 빈 시트 | 표시 | 사용자 안내 문서 |
| 2 | CoAMaster | **테이블 생성 필요** | 표시 | PwC 표준 계정과목 (178개) |
| 3 | CorpMaster | **테이블 생성 필요** | 표시 | 법인 마스터 데이터 |
| 4 | CorpCoA | **테이블 생성 필요** | 표시 | 법인별 CoA 매핑 이력 |
| 5 | BSPL | **테이블 생성 필요** | 표시 | 재무상태표 & 손익계산서 |
| 6 | ADBS | **테이블 생성 필요** | 표시 | 취득/처분 재무상태표 |
| 7 | Verify | 빈 시트 | 표시 | 검증 피벗 테이블 (VBA 자동 생성) |
| 8 | Check | **범위 생성 필요** | 표시 | 워크플로 상태 추적 |
| 9 | HideSheet | **테이블 생성 필요** | 숨김 | 설정 데이터 (결산연월, 환율 등) |
| 10 | DirectoryURL | 빈 시트 | 숨김 | 파일 경로 저장 |
| 11 | Memo | 빈 시트 | 숨김 | 내부 메모 |
| 12 | AddCoA | **테이블 생성 필요** | 숨김* | CoA 입력 (PTB용) |
| 13 | AddCoA_ADBS | **테이블 생성 필요** | 숨김* | CoA 입력 (ADBS용) |
...
CoAMaster 시트의 **A1:K1**에 아래 헤더를 입력하세요:
...
| 컬럼 | 형식 | 설정 방법 |
|------|------|-----------|
| Account (A) | 텍스트 | 컬럼 전체 선택 → 우클릭 → **셀 서식** → **텍스트** |
| Description (B) | 텍스트 | 동일 |
| 연결계정명 (C) | 텍스트 | 동일 |
| Ranking (H) | 숫자 | **숫자** → 소수 자릿수: 0 |
| 금액 (J) | 통화 | **통화** → 기호: ₩, 소수 자릿수: 0 |

**✅ 성공 확인**:
- A1:K1에 테이블 필터 화살표 버튼 표시
- 테이블 이름이 "Master"로 표시 (테이블 도구 → 디자인 탭)
- A2 셀이 빈 상태 (데이터는 나중에 VBA로 삽입)
...
### 4.6 HideSheet 시트 - 설정 테이블 (4개)

HideSheet는 **4개의 별도 테이블**을 포함합니다:
...
**✅ 성공 확인**:
- HideSheet에 4개 테이블 존재
- 테이블 이름: 결산연월, Link, 비경상적, 환율마스터
- 결산연월에 2026, 1 데이터 입력됨
- Link에 SharePoint URL 입력됨
...
**워크플로**:
```
SharePoint 폴더 → Power Query M 코드 → Excel PTB 테이블
```

### 10.2 PTB 연결 생성

#### 단계 1: Power Query 편집기 열기

1. Excel에서 **데이터** 탭 클릭
2. **데이터 가져오기** → **기타 원본에서** → **빈 쿼리**
3. Power Query 편집기 창 열림

#### 단계 2: 고급 편집기 열기

1. Power Query 편집기에서 **홈** 탭 → **고급 편집기** 버튼 클릭
2. 기존 코드 모두 삭제

#### 단계 3: M 코드 붙여넣기

아래 M 코드를 **전체 복사**하여 붙여넣으세요:

```m
let
    // ========== 설정 ==========
    SharePointURL = "https://pwckor.sharepoint.com/sites/KR-ASR-HRE_Consolidation",
    CurrentYear = "2026",
    CurrentMonth = "01",
    ...
    // Financial Data 폴더 필터링
    FilterFinancialData = Table.SelectRows(Source, each Text.Contains([Folder Path], "Financial Data/" & CurrentYear)),

    // 시산표 파일만 필터링 (파일명 패턴: 시산표_HRE-XXX_YYYYMM.xlsx)
    FilterTrialBalance = Table.SelectRows(FilterFinancialData, each Text.StartsWith([Name], "시산표_") and Text.EndsWith([Name], CurrentYear & CurrentMonth & ".xlsx")),

    // ========== Excel 파일 로드 ==========
    AddExcelFile = Table.AddColumn(FilterTrialBalance, "ExcelFile", each Excel.Workbook([Content], null, true)),

    // 시산표 시트 선택
    FilterSheets = Table.SelectRows(AddExcelFile, each [ExcelFile]{[Item="시산표",Kind="Sheet"]}? <> null),
    ExpandSheetData = Table.ExpandTableColumn(FilterSheets, "ExcelFile", {"Data"}, {"SheetData"}),

    // ========== 데이터 확장 ==========
    PromoteHeaders = Table.TransformColumns(ExpandSheetData, {"SheetData", each Table.PromoteHeaders(_, [PromoteAllScalars=true])}),
    ExpandData = Table.ExpandTableColumn(PromoteHeaders, "SheetData", {"법인코드", "계정코드", "계정과목명", "차변", "대변", "기능통화"}, {"법인코드", "계정코드", "계정과목명", "차변", "대변", "기능통화"}),
    ...
    // ========== 계산 컬럼 추가 ==========
    AddNetBalance = Table.AddColumn(ChangedTypes, "차변-대변", each [차변] - [대변], type number),

    // ========== CoA 매핑 컬럼 (VBA가 채움) ==========
    AddPwCCoA = Table.AddColumn(AddNetBalance, "PwC_CoA", each null, type text),
    AddPwCName = Table.AddColumn(AddPwCCoA, "PwC_계정과목명", each null, type text),

    // ========== 필요 컬럼만 선택 ==========
    SelectColumns = Table.SelectColumns(AddPwCName, {"법인코드", "계정코드", "계정과목명", "차변", "대변", "차변-대변", "PwC_CoA", "PwC_계정과목명"})
in
    SelectColumns
```
...
1. **완료** 버튼 클릭
2. Power Query가 SharePoint 연결 시도
3. **인증 필요** 대화상자 표시 → **조직 계정** 선택 → **로그인**
4. Office 365 계정으로 인증
5. 데이터 미리보기 표시 → ✅ 정상

**✅ 성공 확인**:
- Power Query 편집기에 8개 컬럼 표시
- 우측에 적용된 단계 목록 표시 (약 10단계)
- 하단에 데이터 행 표시

#### 단계 5: 테이블로 로드

1. Power Query 편집기에서 **홈** 탭 → **닫기 및 로드** → **닫기 및 로드 위치...**
2. **기존 워크시트** 선택
3. 위치: **BSPL!$A$1** 입력
4. **확인**
...
1. Excel 우측의 **쿼리 및 연결** 창에서 쿼리 우클릭
2. **이름 바꾸기** → `PTB_Query` 입력

**✅ 성공 확인**:
- BSPL 시트의 PTB 테이블에 데이터 채워짐
- 법인코드, 계정코드, 차변, 대변 데이터 표시
- PwC_CoA, PwC_계정과목명은 빈 상태 (VBA가 채울 예정)

### 10.3 Raw_CoA 연결 생성 (선택사항)

**Raw_CoA는 VBA가 자동 관리**하므로, Power Query 연결은 **선택사항**입니다.

만약 SharePoint에 CoA_Master 파일을 업로드하여 공유하려면:

#### CoA_Master 파일 업로드:

1. SharePoint **CoA_Master** 폴더에 `HRE_CoA_Master_v1.00.xlsx` 업로드
2. 파일 내 **Raw_CoA** 시트에 매핑 이력 저장

#### Raw_CoA Query M 코드:

```m
let
    Source = SharePoint.Files("https://pwckor.sharepoint.com/sites/KR-ASR-HRE_Consolidation", [ApiVersion = 15]),
    FilterCoAMaster = Table.SelectRows(Source, each Text.Contains([Folder Path], "CoA_Master") and Text.StartsWith([Name], "HRE_CoA_Master")),
    FirstFile = FilterCoAMaster{0}[Content],
    ExcelFile = Excel.Workbook(FirstFile, null, true),
    RawCoASheet = ExcelFile{[Item="Raw_CoA",Kind="Sheet"]}[Data],
    PromotedHeaders = Table.PromoteHeaders(RawCoASheet, [PromoteAllScalars=true]),
    ChangedTypes = Table.TransformColumnTypes(PromotedHeaders, {
        {"법인코드", type text},
        {"계정코드", type text},
        {"Account", type text},
        {"Variant Type", type text}
    })
in
    ChangedTypes
```
...
### 10.4 로컬 파일 연결 (SharePoint 대안)

SharePoint 사용이 어려운 경우 로컬 폴더 연결:
...
let
    // ========== 로컬 폴더 경로 설정 ==========
    LocalFolder = "C:\HRE\Financial Data\2026\",

    // ========== 폴더 내 모든 Excel 파일 ==========
    Source = Folder.Files(LocalFolder),
    FilterTrialBalance = Table.SelectRows(Source, each Text.StartsWith([Name], "시산표_") and Text.EndsWith([Name], ".xlsx")),

    // ========== Excel 파일 로드 (SharePoint 버전과 동일) ==========
    AddExcelFile = Table.AddColumn(FilterTrialBalance, "ExcelFile", each Excel.Workbook([Content], null, true)),
    // ... (나머지 동일)
in
    SelectColumns
...
**참고**: 로컬 폴더에 모든 법인 시산표 파일을 한 곳에 모아야 합니다.
...
**✅ 성공 확인**:
- 데이터가 다시 로드됨
- 행 개수 하단 상태 표시줄에 표시 (예: "200개 행")

#### VBA 자동 새로고침:

나중에 `QueryRefresh()` 함수로 자동 새로고침됩니다.
